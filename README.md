...
The data can be downloaded from Kaggle's official website from the following link after accepting the site's terms and conditions: [M5 - Forecasting Accuracy](https://www.kaggle.com/competitions/m5-forecasting-accuracy/data?select=sales_train_evaluation.csv)
...
**Before running the codes** to replicate the results, please create an `output/` directory with subdirectories `output/experiment_1/`,`output/experiment_2/`,`output/experiment_3/` and set the FIGURES_PATH parameter in the `src/config.py` file that points to a folder on your local machine for storing plots generated during step 5. Package management is handled by uv, therefore after cloning the repository to your machine please run the following bash command to install all necessary packages and a virtual environment that should be chosen as the kernel when running the below described orchestrator notebooks.
...
5. `04_compare_experiments.ipynb` (Comparison of experiments for the final report, creating graphs)

**Runtime**: approx. 12-14 hours on a Macbook Pro, M3 Pro with macOS 26.3 with each experiment taking approximately 4 hours.
...
The codes are available under a CC-BY 4.0 licence with terms stated in the `LICENCE.txt` file.